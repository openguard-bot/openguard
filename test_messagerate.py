...
class TestMessageRateCog:
    ...
    def test_message_tracking(self):
        ...
        for i in range(5):
            self.cog.message_history[channel_id].append(current_time - timedelta(seconds=i*10))
        
        assert len(self.cog.message_history[channel_id]) == 5, "Message history should contain 5 messages"
        ...
        recent_messages = [
            timestamp for timestamp in self.cog.message_history[channel_id]
            if timestamp >= analysis_cutoff
        ]
        
        # Should have 3 messages (0s, 10s, 20s ago)
        assert len(recent_messages) == 3, f"Expected 3 recent messages, got {len(recent_messages)}"
    ...
    def test_configuration_constants(self):
        ...
        assert self.cog.HIGH_RATE_THRESHOLD > self.cog.LOW_RATE_THRESHOLD, "High rate threshold should be greater than low rate"
        assert self.cog.HIGH_RATE_SLOWMODE > self.cog.LOW_RATE_SLOWMODE, "High rate slowmode should be greater than low rate"
        assert self.cog.LOW_RATE_SLOWMODE > self.cog.NO_SLOWMODE, "Low rate slowmode should be greater than no slowmode"
...
def main():
    ...
    if success:
        ...
        print("You can now test it in your Discord server using the /message ratelimit command.")
    else:
