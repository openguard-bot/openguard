...
async def test_sample_pg(mock_bot):
    ...
    with patch('time.perf_counter', side_effect=[0, 0.001]): # 1ms
        with patch('cogs.ping.get_connection', new_callable=AsyncMock, return_value=mock_pool):
...
async def test_slash_ping_command(mock_bot, mock_ctx):
    ...
    with patch.object(type(mock_bot), 'latency', new_callable=PropertyMock, return_value=0.123), \
