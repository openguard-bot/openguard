...
from bot import bot, get_prefix, MyBot, DualStream, prefix_cache, load_cogs, send_error_dm, catch_exceptions, ERROR_NOTIFICATION_USER_ID, update_bot_guilds_cache, update_launch_time_cache, prefix_update_listener, main, on_error, on_command_error, on_app_command_error, on_ready, on_guild_join, on_guild_remove, on_shard_ready
...
@pytest.mark.parametrize("error_type, user_message_part, should_notify", [
    ...
    (commands.CommandOnCooldown(MagicMock(), 10.0, BucketType.default), "Try again in 10.00 seconds.", False),
...
async def test_on_ready_success(mock_bot):
    ...
    with patch.object(type(mock_bot), 'guilds', [MagicMock(id=1), MagicMock(id=2)]), \
...
async def test_on_ready_sync_failure(mock_bot):
    ...
    with patch.object(type(mock_bot), 'guilds', []), \
         patch('bot.bot', new=mock_bot), \
         patch('builtins.print') as mock_print, \
         patch('bot.send_error_dm', new=AsyncMock()) as mock_send_error_dm, \
...
async def test_update_bot_guilds_cache(mock_bot):
    ...
    with patch.object(type(mock_bot), 'guilds', [MagicMock(id=123), MagicMock(id=456)]), \
         patch('bot.bot', new=mock_bot), \
