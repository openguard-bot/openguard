...
class MessageRateCog(commands.Cog):
    ...
    async def on_message(self, message: discord.Message):
        ...
        current_time = datetime.now(discord.utils.timezone.utc)
    ...
    async def analyze_and_adjust_rate(self, channel: discord.TextChannel, current_time: datetime, analysis_cutoff: datetime):
        ...
        try:
            ...
            recent_messages = [
                timestamp for timestamp in self.message_history[channel_id]
            ...
            if target_slowmode != current_slowmode:
                ...
                log.info(f"Auto-adjusted slowmode for #{channel.name} in {channel.guild.name}: "
                        f"{current_slowmode}s -> {target_slowmode}s (Activity: {activity_level}, "
                        f"Rate: {messages_per_minute} msg/min)")

                # Send notification if configured
                await self.send_rate_change_notification(channel, current_slowmode, target_slowmode,
                                                       messages_per_minute, activity_level)
    ...
    async def send_rate_change_notification(self, channel: discord.TextChannel, old_slowmode: int,
                                          new_slowmode: int, messages_per_minute: int, activity_level: str):
        ...
        try:
            ...
            embed.add_field(
                name="Message Rate",
                value=f"{messages_per_minute} msg/min",
                inline=True
            )
            ...
            embed.timestamp = datetime.now(discord.utils.timezone.utc)
    ...
    message_rate_group = app_commands.Group(name="message", description="Message rate limiting commands")

    @message_rate_group.command(name="ratelimit", description="Configure automatic message rate limiting")
    ...
    async def handle_toggle_action(self, interaction: discord.Interaction, guild_id: int, channel: discord.TextChannel):
        ...
        try:
            ...
            await interaction.followup.send("❌ An error occurred while toggling auto rate limiting.", ephemeral=True)

    async def handle_enable_action(self, interaction: discord.Interaction, guild_id: int, channel: discord.TextChannel):
        ...
        try:
            ...
            await interaction.followup.send("❌ An error occurred while enabling auto rate limiting.", ephemeral=True)

    async def handle_disable_action(self, interaction: discord.Interaction, guild_id: int, channel: discord.TextChannel):
        ...
        try:
            ...
            await interaction.followup.send("❌ An error occurred while disabling auto rate limiting.", ephemeral=True)

    async def handle_status_action(self, interaction: discord.Interaction, guild_id: int, channel: discord.TextChannel):
        ...
        try:
            ...
            recent_messages = [
                timestamp for timestamp in self.message_history[channel.id]
                if timestamp >= analysis_cutoff
            ]
    ...
    async def handle_config_action(self, interaction: discord.Interaction, guild_id: int,
                                 notifications: Optional[bool], notification_channel: Optional[discord.TextChannel]):
