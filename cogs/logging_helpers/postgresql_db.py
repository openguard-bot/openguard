...
async def get_mod_log_channel_id(guild_id: int) -> Optional[int]:
    ...
    try:
        ...
        log.error(f"Error getting mod log channel for guild {guild_id}: {e}")
        return None


async def set_mod_log_channel_id(guild_id: int, channel_id: Optional[int]) -> bool:
    """Sets the mod log channel ID for a guild."""
    try:
        return await set_setting(guild_id, "mod_log_channel_id", channel_id)
    except Exception as e:
        log.error(f"Error setting mod log channel for guild {guild_id}: {e}")
        return False


# Mod Log Enabled Functions

async def is_mod_log_enabled(guild_id: int) -> bool:
    """Checks if mod logging is enabled for a guild."""
    try:
        enabled = await get_setting(guild_id, "mod_log_enabled", True)
        return bool(enabled)
    except Exception as e:
        log.error(f"Error checking mod log enabled status for guild {guild_id}: {e}")
...
async def set_mod_log_enabled(guild_id: int, enabled: bool) -> bool:
    ...
    try:
        ...
        log.error(f"Error setting mod log enabled status for guild {guild_id}: {e}")
        return False
