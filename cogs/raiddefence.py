    ...
    async def stop_raid(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        for user_id in self.suspicious_users:
            try:
                ...
                if user:
                    await interaction.guild.ban(user, reason="Raid Defense: Suspicious join pattern detected", delete_message_days=1)
                    banned_count += 1
...
class RaidDefenceCog(commands.Cog):
    ...
    security_group = app_commands.Group(name="security", description="Security and raid defense commands.")
    raid_subgroup = app_commands.Group(name="raid", description="Raid defense configuration.", parent=security_group)
    
    @raid_subgroup.command(name="config", description="Configure raid defense settings.")
    ...
    async def raid_config(self, interaction: discord.Interaction, enable: bool, threshold: int = 10, timeframe: int = 60):
        ...
        print(f"[RAID DEFENSE] Configuration updated for guild {interaction.guild.name}: enabled={enable}, threshold={threshold}, timeframe={timeframe}")
    ...
    async def on_member_join(self, member: discord.Member):
        ...
        self.join_tracking[guild_id].append({
            ...
            'account_age': (datetime.datetime.utcnow() - member.created_at).total_seconds()
        })
    ...
    async def check_raid_pattern(self, guild: discord.Guild, threshold: int, timeframe: int):
        ...
        recent_joins = [
            join for join in self.join_tracking[guild_id]
    ...
    async def trigger_raid_alert(self, guild: discord.Guild, suspicious_users: list, total_joins: int):
        ...
        print(f"[RAID DEFENSE] Potential raid detected in guild {guild.name}: {total_joins} joins, {len(suspicious_users)} suspicious")
        ...
        if general_channel and general_channel.permissions_for(guild.me).send_messages:
            try:
                await general_channel.send(f"🚨 **RAID ALERT** - {guild.owner.mention if guild.owner else 'Server Owner'}", embed=embed, view=view)
