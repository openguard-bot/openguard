...
from database.operations import (
    ...
    add_user_infraction, get_user_infractions, clear_user_infractions,
    
    # Appeals operations
    create_appeal, get_appeal, update_appeal_status, get_user_appeals,
    
    # Global bans operations
    add_global_ban, remove_global_ban, is_globally_banned, get_all_global_bans,
    
    # Moderation logs operations
    add_mod_log, get_mod_log, update_mod_log_reason, get_user_mod_logs, get_guild_mod_logs,
    
    # Guild settings operations
    get_guild_setting, set_guild_setting,
    
    # Log event toggles operations
    get_log_event_enabled, set_log_event_enabled, get_all_log_event_toggles,
    
    # Bot detection config operations
    get_botdetect_config, set_botdetect_config, get_all_botdetect_config,
    
    # User data operations
    get_user_data, set_user_data, update_user_data_field, delete_user_data
)
...
async def test_guild_config_operations():
    ...
    try:
        ...
        print_test_result("Get Guild Config", success, f"Expected: {test_value}, Got: {retrieved_value}")
        if not success:
            all_passed = False
...
async def test_appeals_operations():
    ...
    try:
        ...
        if appeal_id:
            ...
            print_test_result("Get User Appeals", success, f"Count: {len(user_appeals)}")
...
async def test_moderation_logs_operations():
    ...
    try:
        ...
        if case_id:
            ...
            print_test_result("Get Guild Mod Logs", success, f"Count: {len(guild_logs)}")
...
async def test_settings_operations():
    ...
    try:
        ...
        success = retrieved_value == test_value
        print_test_result("Get Guild Setting", success, f"Expected: {test_value}, Got: {retrieved_value}")
        if not success:
            all_passed = False
...
async def test_log_event_toggles_operations():
    ...
    try:
        ...
        print_test_result("Get Log Event Enabled", success, f"Expected: False, Got: {is_enabled}")
        ...
        print_test_result("Get All Log Event Toggles", success, f"Count: {len(all_toggles)}")
...
async def test_botdetect_config_operations():
    ...
    try:
        ...
        success = retrieved_value == test_value
        print_test_result("Get Botdetect Config", success, f"Expected: {test_value}, Got: {retrieved_value}")
        if not success:
            all_passed = False
        ...
        print_test_result("Get All Botdetect Config", success, f"Count: {len(all_config)}")
        if not success:
            all_passed = False
...
async def test_user_data_operations():
    ...
    try:
        ...
        print_test_result("Get User Data", success, f"Expected: {test_data}, Got: {retrieved_data}")
        
        # Test updating user data field
        success = await update_user_data_field(test_user_id, "department", "Engineering")
...
async def main():
    ...
    print_colored("Testing all database operations to verify migration success", Colors.CYAN)
    print()
    ...
    if missing_vars:
        ...
        print_colored("Please run the setup_postgresql.sh script first or set the variables manually", Colors.YELLOW)
        return
    ...
    try:
        ...
        print_colored(f"Passed: {passed_tests}/{total_tests}", Colors.GREEN if passed_tests == total_tests else Colors.YELLOW)
        
        if passed_tests == total_tests:
            print_colored("🎉 All tests passed! PostgreSQL migration is successful.", Colors.GREEN)
        else:
            print_colored(f"⚠️  {total_tests - passed_tests} test(s) failed. Please check the issues above.", Colors.YELLOW)
